...
This method has been used with great success \citep{johnston&doering2009, verzicco&camussi1997}, but it is not without its faults.  
...
For example, past studies have used initial conditions from a linear eigenvalue solve \citep{hurlburt&all1984} in plane-parallel studies, or axisymmetric solutions in studies of convection in three-dimensional (3D)  cylinders \citep{verzicco&camussi1997}. 
...
Rather, either through knowledge of low-Ra solutions \citep{couston&all2017} or broader convective theories such as Mixing Length Theory \citep{brandenburg&all2005}, initial conditions can be adjusted such that the stratification within the convective domain is closer to a relaxed state than the largely unstable hydrostatic state.
...
Some modern studies do just that \citep{featherstone&hindman2016a}.
Such evolution is computationally \emph{expensive}, and state-of-the-art simulations at the highest values of Ra can only reasonably be run for hundreds of freefall timescales \citep{stevens&all2011}, much less the thousands or millions of freefall times required for thermal relaxation.
...
As such, it is possible to measure the statistically stationary mean value of Nu after only a few hundred simulated freefall times, as has been done previously \citep{stevens&all2010}. 
...
Our choice of no-slip boundary conditions prevents the fluid from entering a full domain shearing state \citep{goluskin&all2014}, and the oscillatory motions of the plumes are a long-lived, stable phenomenon. 
...
Thus, the inverse scaling of average temperature and Nu that we find here is expected for thermally equilibrated solutions \citep{otero&all2002}.
...
The probability distribution functions (PDFs) of point-by-point temperature measurements are compared for the two runs in Fig.~\ref{fig:temp_comparison}(b)\footnote{

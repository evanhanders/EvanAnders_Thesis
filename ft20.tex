\chapter{Convective dynamics with mixed temperature boundary conditions: why thermal relaxation matters and how to accelerate it}
\label{ch:FT20}

\section{Preamble}
This manuscript in Sec.~\ref{sec:ft20_manuscript} appears as it was submitted to Physical Review Fluids on Feb.~28, 2020.
Its initial version can be found on the arXiv \citep{anders&all2020}, and the text here contains minor modifications suggested by the Thesis Committee.
...
The rotating cases in subsection \ref{sec:ft20_rotating_results} employ stress-free, impenetrable boundaries,
\begin{equation}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsubsection{Nondimensional Output Quantities}
...
\caption[Scalar time traces and parameter space scalings for FT and TT simulations]{ 
	(Left three panels) Time traces of scalar quantities in an FT (orange, Ra$_{\partial_z T} = 4.83 \times 10^{10}$) and TT (purple, Ra$_{\Delta T} = 10^9$) simulation are shown.
	...
	The yellow trace shows the evolution of an FT case with Ra$_{\Delta T} = 10^8$ (Ra$_{\partial_z T} = 2.61 \times 10^{9}$).
...
\vspace{-0.25cm}
\subsubsection{Evolved Structure}
\vspace{-0.5cm}
\begin{figure}
...
\caption[1D profiles of temperatures and fluxes in TT and FT simulations]{ 
	(Left three panels) We compare time- and horizontally-averaged profiles from an FT (orange) and TT (purple) simulation at Ra$_{\Delta T} = 10^{9}$ (Ra$_{\partial_z T} = 4.83 \times 10^{10}$).
...
\caption[PDF and snapshot examination of asymmetries in FT simulations]{ 
	(Left panel) PDFs of temperature measurements of a TT-to-FT (orange) and TT (purple) case with Ra$_{\Delta T} = 10^{10}$ (Ra$_{\partial_z T} = 9.51 \times 10^{11}$) are displayed.
...
We study a TT case at Ra$_{\Delta T} = 2.75\times 10^9$, and an FT case at $\text{Ra}_{\partial_z T} = 2.1 \times 10^{10}$ (the supercriticality of the TT case is $\sim 3$)\footnote{
At these parameters, we would be in the cellular convection regime of \citet{stellmach&all2014}, but our dynamics are different due to our choice of stress free boundary conditions.
}.
...
\caption[Time traces, parameter space plot, and snapshots showing evolution of rotating FT simulation]{ 
	(Left three panels) Time traces of scalar quantities in an FT (orange) and TT (purple) simulation are shown with Ra$_{\Delta T} = 2.75 \times 10^9$ (Ra$_{\partial_z T} = 2.1 \times 10^{10}$) and Ek = $10^{-6}$.
...
However, some of those studies involve internal heating profiles, and it is unclear if the rapid relaxation we observe here in TT simulations would carry over to simulations of internally heated convection.
Regardless, preliminary results in convection in 3D spherical shells suggests that FF boundaries converge faster than FT boundaries (Matilsky 2020, private communication).
...
\begin{table}[ht]
\caption[Table of simulations for Anders et al.~2020 (submitted)]{
	...
	The ``Nu comp'' values are comparison Nu values reported in \citet{zhu&all2018}.

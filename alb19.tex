\chapter{Entropy Rain: Dilution and Compression of Thermals in Stratified Domains}
\label{ch:alb19}
...
He conducted a couple of initial simulations of these vortices and reported briefly on them in the supplemental materials of \cite{brandenburg2016}.
The results of those initial simulations didn't look too promising for the ``Entropy Rain'' idea -- that is, they compressed with increasing atmospheric density, as expected.
...
In this paper, we extend \cite{lecoanet&jeevanjee2018} (hereafter \LJ) to study the propagation of low-Mach number, low-entropy thermals in stratified domains. 
...
This subsection parallels a similar analysis for the Boussinesq case, presented in \cite{mckim&all2019}.

The hydrodynamic impulse is defined as \cite{shivamoggi2010},
\begin{equation}
...
However, one can also show that the surface terms correspond to the momentum outside the volume $\mathcal{V}$ \cite[e.g.][]{Akhmetov2009}.
...
\end{equation}
where $k$ encompasses the ``virtual mass effect'' from the environmental fluid moving together with the thermal \cite{tarshish&all2018}.
...
\subsubsection{Model of thermal evolution}
\begin{figure}[p!]
...
\subsubsection{2D Anelastic Simulations}
\label{sec:alb19_anelastic}
The LBR anelastic equations are \cite{lecoanet&all2014},
\begin{gather}
...
\subsubsection{3D Fully Compressible Simulations}
In order to verify our 2D anelastic simulations, we also simulate thermals with the 3D Navier Stokes equations. 
We use the $(T, \ln\rho)$ formulation of the equations \cite{lecoanet&all2014, anders&brown2017},
\begin{gather}
...
The new parameter $\epsilon = -u_{\text{th}}^2/\grad_{\text{ad}}$ is the magnitude of entropy perturbations and sets the Mach number of the thermal flows analagously to the adiabatic excess in polytropic convection \cite{anders&brown2017}; we use $\epsilon = 10^{-4}$ in this work. 
...
All of the code used to perform the simulations in this work can be found online in the supplementary materials in a Zenodo repository \cite{supp_andersetall2019b} at \url{https://doi.org/10.5281/zenodo.3311894}.
...
The values of the parameters described in subsection \ref{sec:theory} for each of our simulations are presented in table \ref{table:parameters}.
...
\subsection{Appendix A: Thermal Measurements}
...
\subsection{Appendix B: Thermal Tracking Algorithm}
...
We can the spherical harmonic degree of these thermals as $\ell \approx R_{\text{CZ}}/ r_{\text{th}} \approx 10^{5}$.
